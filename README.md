...
Feet: (Gridfinity only) Generates the complex base geometry.

Floor: Adds the solid bottom plate.

Walls: Extrudes the main body based on defined height or unit count.

Shoulder/Rim: Calculates the stacking shoulder height.

Lip/Lid: Adds the final stacking lip or lid rails.
...
Select Mode: Choose between Inch, mm, or Gridfinity via the top segmented control.
...
Adjust Structure: Modify wall and floor thickness if necessary (defaults are optimized for FDM printing).

Add Features: Enable lids (Standard Mode) or switch between Bin/Frame types (Gridfinity Mode).

Export: Click "Download .STL" to save the model.

Installation

This component is designed to run in a React environment.

Ensure three and react are installed.

Import the App component.

The component is self-contained and handles its own resize observers and rendering loops.
